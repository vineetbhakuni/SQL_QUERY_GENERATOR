-- ============================================================
-- HR sample database — converted from MySQL to PostgreSQL
-- Ready to run in the Supabase SQL Editor.
--
-- Notes on the conversion:
--   * Removed `CREATE DATABASE test;` and `USE test;`
--     (Supabase already provides the database; objects go in
--      the `public` schema by default).
--   * `INT(11)`            -> INTEGER        (Postgres has no display width)
--   * `AUTO_INCREMENT`     -> GENERATED BY DEFAULT AS IDENTITY
--   * `DECIMAL(8,2)`, VARCHAR, CHAR, DATE   -> unchanged
--   * Sequences are resynced at the end because rows were
--     inserted with explicit primary-key values.
-- ============================================================

-- Optional: start clean if re-running.
DROP TABLE IF EXISTS dependents, employees, departments, jobs, locations, countries, regions CASCADE;
...
CREATE TABLE dependents (
    dependent_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
-- ------------------------------------------------------------
-- Resync identity sequences so future inserts (without an
-- explicit id) continue after the highest value already used.
-- ------------------------------------------------------------
SELECT setval(pg_get_serial_sequence('regions',     'region_id'),     (SELECT MAX(region_id)     FROM regions));
...
SELECT setval(pg_get_serial_sequence('dependents',  'dependent_id'),  (SELECT MAX(dependent_id)  FROM dependents));